...
- 做多：最近 3 根已收盘 15m K 线内至少出现一次收盘价由不高于当时 EMA200 到高于 EMA200 的向上突破，且当前收盘价仍高于当前 EMA200；当前 EMA200 高于 5 根 K 线前的 EMA200；当前 15m USDT 成交额严格高于前 20 根平均成交额的 `1.1` 倍，不再要求高于上一根成交额；RSI(14) 严格高于上一根 RSI，不设置 RSI 上限或最低值。
- 停止开多：已发做多或恢复做多信号后，`OI / 市值 ≤ 90%`，或 15m 收盘价不高于 EMA200 时进入该状态；该状态表示禁止继续开多或补仓，并继续显示在 Web 页面，但不发送单币企业微信通知。EMA200 斜率、价格校正 OI 增长、成交额突破和 RSI 回升仅用于确认开多，不会因随后不再满足而单独触发停止开多。
- 恢复做多：停止做多后，最近 3 根已收盘 15m K 线内再次出现新的 EMA200 向上突破并重新满足全部开多条件，才推送一次【交易信号：恢复做多】。该信号仅适用于已经平仓后的重新开仓，不作为亏损仓位补仓依据。
...
- 动态冷却：必须退出时用当前 `ATR/收盘价` 对比前 96 个该指标的中位数；比值 `≤0.8` 冷却 3 根，`0.8～1.2` 冷却 4 根，`>1.2` 冷却 6 根 15m K 线。冷却结束后仍须出现新的完整开多条件，且 EMA200 向上突破不能发生在冷却结束前。
