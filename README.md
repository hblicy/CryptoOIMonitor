...
- Web 页面“交易条件扫描”会列出所有 `OI / 市值 > 110%` 的标的，分为可以做多、停止做多、必须退出和 K 线异常，并展示 RSI、收盘价、EMA200、OI / 市值和对应原因。
- 当“可以做多”或“停止做多”有新增币种时，企业微信立即推送当前两类完整列表；没有新增时，每 1 小时推送一次当前列表。列表只显示币种；K 线扫描存在失败时，不推送不完整列表。
