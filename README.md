...
- 停止开多：已开多后，OI / 市值降至 `<= 100%`、RSI(14) 超过 50，或 15 分钟收盘价低于 EMA200 时推送停止开多提示；OI / 市值条件会立即生效，不等待 K 线请求。
- 杠杆参考：2-3 倍；信号不包含自动下单或仓位金额。
- 同一币种在停止开多条件触发前不重复推送；提示停止开多后，RSI(14) 回落至 50 以下且收盘价重回 EMA200 上方时可再次推送做多。
- Web 页面“交易条件扫描”会列出所有 `OI / 市值 > 100%` 的标的，分为可以做多、停止做多和 K 线异常，并展示 RSI、收盘价、EMA200、OI / 市值和停止原因。
