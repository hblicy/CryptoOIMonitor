...
`OI / 市值 > 200%` 时，页面标识为埋伏候选区；该标签只表示高风险、仅用于页面观察，不发送企业微信消息，也不作为开多上限或退出条件。
...
- 前置条件：当前快照完整且 `OI / 市值 > 90%`，不设 OI/MC 上限；`> 200%` 仍显示为埋伏候选区，但执行与其他候选相同的完整入场条件。系统在目标时间前后 5 分钟内选取距 15 分钟前最近的完整快照，并用 `当前聚合 OI USD ÷ 当前 Binance 收盘价` 与 `参考聚合 OI USD ÷ 上一根 Binance 收盘价` 比较；价格校正后的持仓量必须增加。缺失历史快照或币种时不发开多信号，并在页面显示原因。
- 做多：最近 3 根已收盘 15m K 线内至少一次由收盘价不高于当时 EMA200 到高于 EMA200 的向上突破，且当前收盘价仍高于当前 EMA200；当前 EMA200 高于 5 根 K 线前的 EMA200；当前 15m USDT 成交额严格高于前 20 根平均成交额的 `2` 倍，不再要求高于上一根成交额；RSI(14) 严格高于上一根 RSI，不设置 RSI 上限或最低值。
- 停止开多：已发做多或恢复做多信号后，`OI / 市值 ≤ 90%` 或 15m 收盘价不高于 EMA200 时进入该状态；该状态表示禁止继续开多或补仓，并继续显示在 Web 页面，但不发送单币企业微信通知。`OI / 市值 > 200%` 本身不会触发停止开多或必须退出。EMA200 斜率、价格校正 OI 增长、成交额突破和 RSI 回升仅用于确认开多，不会因随后不再满足而单独触发停止开多。
- 恢复做多：停止做多后，最近 3 根已收盘 K 线内再次出现 EMA200 向上突破并重新满足全部开多条件，才推送一次【交易信号：恢复做多】。该信号仅适用于已经平仓后的重新开仓，不作为亏损仓位补仓依据。
...
- 动态冷却：必须退出时用当前 `ATR/收盘价` 对比前 96 个该指标的中位数；比值 `≤0.8` 冷却 3 根，`0.8～1.2` 冷却 4 根，`>1.2` 冷却 6 根 15m K 线。冷却结束后仍须在最近 3 根已收盘 K 线内出现新的 EMA200 向上突破并满足全部开多条件，且该突破不能发生在冷却结束前。
...
- Web 页面“交易条件扫描”会列出所有 `OI / 市值 > 90%` 的标的，统一加载 Binance K 线并执行相同的 OI 增长、EMA、成交额、RSI 和冷却判断，分为可以做多、停止做多、必须退出和 K 线异常。已有仓位仍优先执行 ATR、移动保护和 EMA 必须退出风控。主表的重点关注门槛仍为 `> 110%`，埋伏候选区仍为 `> 200%`，但这两个标签不改变交易状态机。
