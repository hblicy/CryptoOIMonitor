...
- 15m EMA200 使用最近 1001 根已收盘 K 线计算，其中 1000 根用于预热，以对齐 Binance 图表口径。
- 前置条件：当前快照完整、`OI / 市值 > 100%`，且当前聚合 OI 高于约 15 分钟前的完整快照。系统在目标时间前后 5 分钟内选取时间最接近的完整快照；缺失历史快照或币种时不发开多信号，并在页面显示原因。
- 做多：上一根已收盘 15m 收盘价不高于当时 EMA200，当前已收盘 15m 收盘价高于当前 EMA200；当前 15m USDT 成交额高于上一根；RSI(14) 在 `[35, 50)` 且高于上一根已收盘 K 线的 RSI。
- 停止开多：已发做多信号后，`OI / 市值 ≤ 100%`、RSI(14) 达到 50，或 15m 收盘价不高于 EMA200 时推送；该状态表示禁止继续开多或补仓。首次上穿、聚合 OI 增长、15m 成交额增长和 RSI 回升仅用于确认开多，不会因随后不再满足而单独触发停止开多。
...
- 同一币种在停止开多条件触发前不重复推送；停止后再次出现完整的首次上穿信号并满足全部开多确认条件，才可再次推送做多。
- Web 页面“交易条件扫描”会列出所有 `OI / 市值 > 100%` 的标的，分为可以做多、停止做多、必须退出和 K 线异常，并展示 RSI、收盘价、EMA200、OI / 市值、聚合 OI 与 15m 成交额变化及对应原因。主表的重点关注门槛仍为 `> 110%`，埋伏候选区仍为 `> 200%`。
